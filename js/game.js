...
function submitSelectedAnswer(auto=false){
  if(answered||selectedOption===null)return;
  doAnswer(selectedOption,auto);
}

function doAnswer(i,autoSubmitted=false){
  if(answered)return;
  answered=true;
  clearInterval(timerInterval);
  ...
  const submitBtn=document.getElementById('submit-btn');
  submitBtn.disabled=true;
  submitBtn.textContent=autoSubmitted?'Auto-submitted':'Submitted';
  const fb=document.getElementById('feedback');
  const aiMissed=aiMisses.includes(kick);
  if(correct){
...
function showEnd(){
  ...
  const lbTitle=document.getElementById('end-lb-title');
  if(lbTitle)lbTitle.textContent=`🏆 ${DIFF_NAMES[diff]} Leaderboard`;
  renderLeaderboard('lb-list',DIFF_NAMES[diff]);
}
